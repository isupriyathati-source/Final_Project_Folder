# D:\Project\MeanShift\Pages\Bulk_Analysis.py
...
# ==================== MODEL LOADING ====================
try:
    with open("model_files/ms_model.pkl", "rb") as f:
        ms_model = pickle.load(f)
    with open("model_files/scaler.pkl", "rb") as f:
        scaler = pickle.load(f)
except FileNotFoundError:
    st.error("Model files not found. Please ensure 'ms_model.pkl' and 'scaler.pkl' are in 'model_files/'.")
    st.stop()
...
st.markdown("### Upload a CSV file to automatically segment your customers using MeanShift")
...
if uploaded_file:
    ...
    labels = ms_model.predict(X_scaled)
    df['Cluster'] = labels
    
    st.success(f"✅ Clustering complete! Number of clusters: {len(np.unique(labels))}")
    
    # Cluster Summary (exclude noise if any, though MeanShift rarely produces -1)
    ...
    ax.set_title("MeanShift Customer Segments")
    ax.legend()
    st.pyplot(fig)
    ...
    st.download_button("📥 Download Processed Data with Cluster Names", csv, "meanshift_processed_data.csv", "text/csv")
    
    # Store in session for other pages
    st.session_state.df_clusters = df
    
    log_activity(st.session_state["username"], "bulk_analysis", {"records": len(df), "clusters": len(np.unique(labels))})